...
_CATEGORY_RULES = [
    ...
    #
    # ORDER MATTERS — first match wins. Type IV restriction is listed before
    # Restriction-Modification so modification-dependent systems are not
    # swallowed by the generic RM rule.
    #
    # Two corrections relative to the original mapping, both of which would
    # otherwise corrupt the A4 mechanism partition:
    #
    #   * DRT (defense-associated reverse transcriptase) was classified as
    #     Restriction-Modification. DRTs are retroelement-based, mechanistically
    #     unrelated to RM, and would have entered the "predicted dsDNA-
    #     restricting" group by mistake. Moved to Retron-like.
    #   * McrBC / Mrr are modification-DEPENDENT (Type IV) endonucleases, not
    #     classical RM. Still dsDNA-cleaving, so still in the predicted group,
    #     but they deserve their own label because their self/non-self logic is
    #     inverted relative to Type I-III.
    ("Type-IV-Restriction",       [r"^mcrbc", r"^mcr[_\-]?[a-z]", r"^mrr",
                                   r"^gmrsd", r"^sspe", r"^sspbcde",
                                   r"type_iv_restriction"]),
    ("Restriction-Modification", [r"^rm[_\-]?", r"^type_i+_", r"type_iii",
                                   r"^bregab"]),
    ("CRISPR-Cas",                [r"^crispr", r"^cas\b", r"^cas[_\-]?[0-9]"]),
    ("Retron",                    [r"^retron", r"^drt"]),
