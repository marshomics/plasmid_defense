...
def run_tier1(binary_df: pd.DataFrame, prevalence_df: pd.DataFrame,
    ...
    for covariate_mode in config.covariate_modes:
        cov_base = list(config.resolve_covariates(
            config.covariate_columns_for_mode(
                covariate_mode, include_plasmid_count=False), working_binary))
        cov_binary = list(config.resolve_covariates(
            config.covariate_columns_for_mode(
                covariate_mode, include_plasmid_count=True), working_binary))
        ...
        for outcome_label in sorted(outcome_spec.keys()):
            ...
            # Threading backend for the same reason as tier3_sensitivity:
            # loky's process-based backend has broken on some conda Python
            # 3.10 builds where joblib.externals.loky.backend.synchronize
            # is missing. Firth uses numpy/statsmodels which release the
            # GIL during heavy operations, so threading is fine here.
